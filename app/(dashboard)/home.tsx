...
const Home = () => {
  ...
  const VehicleCard = ({ item, width = 200, showDistance = false }: any) => (
    <TouchableOpacity 
        ...
        style={{ width: width }}
    >
        <Image source={{ uri: item.imageUrl }} className="w-full h-36 bg-gray-200" resizeMode="cover" />
        <View className="p-3">
            <View className="flex-row justify-between items-center mb-1">
                <Text className={`text-[10px] font-bold px-2 py-1 rounded uppercase ${isDark ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-500'}`}>{item.vehicleBrand}</Text>
            ...
            {showDistance && item.distance !== undefined && (
                <View className="flex-row items-center mt-1">
                    ...
                    <Text className="text-xs ml-1 font-bold text-red-500">{item.distance < 1 ? "Nearby (<1km)" : `${item.distance.toFixed(1)} km`}</Text>
                </View>
            )}
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={iconColor} />}>
        ...
        <View className='px-6 flex flex-row mt-4 justify-between items-center'>
          <View className='flex flex-row items-center gap-2'>
            <Image source={require("../../assets/images/icon.png")} className={`w-10 h-10 bg-white rounded-full p-4 ${isDark ? 'border border-gray-200' : 'border border-gray-700'}`} resizeMode="contain" />
        ...
        <View className='px-6 mt-6'>
          <View className='relative'>
            <TextInput 
                placeholder='Search brands, models...' placeholderTextColor={placeholderColor}
            ...
            <FlatList horizontal data={CATEGORIES} keyExtractor={i => i.id} showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 24, gap: 15 }}
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => setSelectedCategory(item.id)} className="items-center">
                        <View className={`w-14 h-14 rounded-2xl items-center justify-center mb-2 ${selectedCategory === item.id ? (isDark ? 'bg-white' : 'bg-black') : (isDark ? 'bg-gray-800' : 'bg-gray-100')}`}>
                            <item.Library name={item.iconName} size={24} color={selectedCategory === item.id ? (isDark ? 'black' : 'white') : (isDark ? 'white' : 'gray')} />
                        </View>
        ...
        {loading ? <ActivityIndicator size="large" color={iconColor} className="mt-10" /> : (
            <>
                ...
                {nearbyVehicles.length > 0 ? (
                    ...
                    <View className="mt-6 px-6"><Text className={`text-xl font-bold mb-2 ${textMain}`}>Nearby</Text><Text className={`${textSub} text-sm italic`}>No vehicles found within 20km.</Text></View>
                ))}
                        ...
                        <TouchableOpacity key={item.id} activeOpacity={0.9} onPress={() => router.push({ pathname: "/add/[id]", params: { id: item.id } })} className={`${bgCard} rounded-2xl shadow-sm border ${borderCol} overflow-hidden mb-4 flex-row h-32`}>
                            ...
                            <View className="flex-1 p-3 justify-between">
                                ...
                                <View className="flex-row justify-between items-end">
                                    <Text className={`text-sm font-bold ${textMain}`}>Rs.{item.price}<Text className={`text-[10px] font-normal ${textSub}`}>/day</Text></Text>
                                    <View className={`px-2 py-1 rounded border ${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}><Text className={`text-[10px] font-bold ${textSub}`}>Book Now</Text></View>
                                </View>
                            </View>
                        </TouchableOpacity>
                    ))}
